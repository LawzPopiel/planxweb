...
(function(process) {
  ...
  (function(window, undef) {
    ...
    var gid = 1,
        ...
        html5 = !!(File && (FileReader && (window.Uint8Array || FormData || XMLHttpRequest.prototype.sendAsBinary))) && !(/safari\//i.test(userAgent) && !/chrome\//i.test(userAgent) && /windows/i.test(userAgent)),
        ...
        api = {
          ...
          upload: function(options) {
            ...
            var proxyXHR = new api.XHR(options),
    ...
    api.addInfoReader(/^image/, function(file, callback) {
    ...
    api.event.dnd = function(el, onHover, onDrop) {
      ...
      if (FileReader) {
        _on(el, 'dragenter dragleave dragover', onHover.ff = onHover.ff || function(evt) {
          var types = _getDataTransfer(evt).types,
    ...
    XHR.prototype = {
      ...
      _send: function(options, data) {
        ...
        if (data.nodeName) {
          ...
          xhr.innerHTML = '<form target="' + uid + '" action="' + url + '" method="POST" enctype="multipart/form-data" style="position: absolute; top: -1000px; overflow: hidden; width: 1px; height: 1px;">' + '<iframe name="' + uid + '" src="javascript:false;" onload="' + onloadFuncName + '()"></iframe>' + (jsonp && (options.url.indexOf('=?') < 0) ? '<input value="' + uid + '" name="' + jsonp + '" type="hidden"/>' : '') + '</form>';
          ;
    ...
    api.support.flash && (0 || !api.html5 || !api.support.html5 || (api.cors && !api.support.cors) || (api.media && !api.support.media)) && (function() {
      var _attr = api.uid(),
          ...
          flash = {
            ...
            select: function(evt) {
              try {
                var inp = flash.getInput(evt.flashId),
                    uid = api.uid(inp),
            ...
            patch: function() {
              ...
              api.Image && _inherit(api.Image.prototype, {
                get: function(fn, scaleMode) {
                  this.set({scaleMode: scaleMode || 'noScale'});
              ...
              api.Image && _inherit(api.Image, {fromDataURL: function(dataURL, size, callback) {
                  if (!api.support.dataURI || dataURL.length > 3e4) {
                    _makeFlashImage(api.extend({scale: 'exactFit'}, size), dataURL.replace(/^data:[^,]+,/, ''), function(err, el) {
                      callback(el);
                    });
                  } else {
              ...
              _inherit(api.Form.prototype, {toData: function(fn) {
              ...
              _inherit(api.XHR.prototype, {_send: function(options, formData) {
                  if (formData.nodeName || formData.append && api.support.html5 || api.isArray(formData) && (typeof formData[0] === 'string')) {
                    return this.parent.apply(this, arguments);
                  }
                  var data = {},
                      files = {},
  ...
  (function(window, jQuery, api) {
    ...
    if (api.support.flash && (api.media && !api.support.media)) {
      (function() {
        ...
        var flash = api.Flash;
        api.extend(api.Flash, {patchCamera: function() {
            ...
            api.extend(api.Camera.prototype, {
              ...
              start: function(callback) {
                var _this = this;
                flash.cmd(this._id(), 'camera.on', {callback: _wrap(function _(evt) {
                    _unwrap(_);
                    if (evt.error) {
