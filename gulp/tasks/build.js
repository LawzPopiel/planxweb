...
gulp.task('bundle', ['jshint'], (cb) => {
    ...
    const ENV = !!util.env.env ? util.env.env : 'PROTOTYPE';
    console.log(ENV.toLowerCase() === 'prototype');
    console.log(ENV.toLowerCase());
    const Builder = require('systemjs-builder');
    const builder = new Builder();
    const inputPath = 'src/app/app';
    const outputFile = `${path.tmp.scripts}build.js`;
    const outputOptions = {sourceMaps: true, config: {sourceRoot: path.tmp.scripts}, conditions: { 'ENV|mock': ENV.toLowerCase() === 'prototype', 'ENV|environment': ENV.toLowerCase()} };
