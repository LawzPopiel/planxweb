...
gulp.task('test:e2e', ['webdriver_update'], () => {
    console.log(path.test.e2e);
    return gulp.src(path.test.e2e)
        .pipe(protractor.protractor({
            configFile: path.test.config.protractor,
            args: ['--baseUrl', E2E_DEVELOPMENT_URL + ":4444", '--capabilities.browserName', BROWSERS.toLowerCase()] // protractor only accept lowercase browser name
        }))
        //.on('error', function () {
