...
const argv = util.env;
const LOG = util.log;
const ENV = !!argv.env ? argv.env : 'PROTOTYPE';
const COLORS = util.colors;

if(!ENV.match(new RegExp(/prod|dev|prototype|DEV|PROTOTYPE|PROD/))) {
    LOG(COLORS.red(`Error: The argument 'env' has incorrect value ${ENV}! Usage: --env=(DEV|PROTOTYPE|PROD)`));
    process.exit(1);
}
...
gulp.task('sass', () => {
    ...
    const errLogToConsole = ENV === 'dev' || ENV === 'DEV'|| ENV === 'prototype'|| ENV === 'PROTOTYPE';
