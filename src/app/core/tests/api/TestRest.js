...
import TestDataHandler from '../common/TestDataHandler';
import AbstractApiTest from '../abstracts/AbstractApiTest';
import DescribeBlock from '../common/DescribeBlock';
...
class TestRest extends AbstractApiTest {
    initialize(){
        ...
        this.Describe = new DescribeBlock("TESTING " + API_BASE);
    ...
    setIDFromResponse(response){
        ...
        } else if(_.isObject(response) && _.has(response, ID_PROP)) {
            this.opts.id = response[ID_PROP];
            if(!_.isNumber(response[ID_PROP])){
                this.opts.id = parseInt(response[ID_PROP], 10);
            }
        }
    }
    ...
    callServer(methodType, changeFixture, params, callback){
        ...
        this.Describe.before((done)=>{
            ...
            if(changeFixture) {
                ...
                if(_.has(newFixture, ID_PROP)) {
                    id = newFixture[ID_PROP];
                }
            ...
            switch(methodType) {
                case "GET":
                    if(!id && _.isObject(newFixture) && newFixture[ID_PROP]) {
                        id = newFixture[ID_PROP];
                    }
                ...
                case "POST":
                    ...
                    if(_.isObject(newFixture) && newFixture[ID_PROP]) {
                        delete newFixture[ID_PROP];
                    }
    ...
    expectID(Describe, testObj){
        ...
        Describe.it("should have ID", ()=>{
            expect(testObj.response[ID_PROP]).to.be.a("number");
        });
    }
